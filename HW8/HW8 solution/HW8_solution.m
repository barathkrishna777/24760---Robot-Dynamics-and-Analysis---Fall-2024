...
% Physics
syms m_l1 m_l2 L I_l1 I_l2 w m_o I_o g real
...
V = m_l1*g*(w/2+L/2*sin(theta1)) + m_l2*g*(w/2 + L*sin(theta1) + L/2*sin(theta1+theta2)) + m_o*g*o_y;
N = jacobian(V, q)';
Y = [tau_1;tau_2;0;0;0];
...
EOM = M*ddq + C*dq + N + A'*lambda - Y;

%% Problem 1.2

% Calculate block matrix inverse
blockMinv = inv([M A';A zeros(c,c)]);
...
ddq_massive = M_dag*(Y - C*dq - N) - A_dag'*dA*dq;

% Numerical evaluation
symbols = [tau_1; tau_2; L; m_l1; m_l2; I_l1; I_l2; w; m_o; I_o; g; q; dtheta1; dtheta2; do_x; do_y; do_theta];
values = [200; 0; 0.1; 1; 1; 8.33e-4; 8.33e-4; 0.2; 24; 0.16; 9.81; pi/2; -pi/2; 0.1; 0.2; 0; 0; 0; 0; 0; 0];
...
EOM_massless = subs(EOM, [m_l1, m_l2, I_l1, I_l2], [0,0,0,0]);
...
% Numerical evaluation
symbols = [tau_1; tau_2; L; m_l1; m_l2; I_l1; I_l2; w; m_o; I_o; g; q; dtheta1; dtheta2; do_x; do_y; do_theta];
values = [200; 0; 0.1; 0; 0; 0; 0; 0.2; 24; 0.16; 9.81; pi/2; -pi/2; 0.1; 0.3; 0; 0; 0; 0; 0; 0];

% In the massless case, M is not invertable so you have to do the block matrix inverse approach
...
EOM_frictionless = M*ddq + C*dq + N + A_frictionless'*lambda_frictionless - Y;

%% Problem 1.6

% Compute massless and frictionless EOM
EOM_massless_frictionless = subs(EOM_frictionless, [m_l1, m_l2, I_l1, I_l2], [0,0,0,0])

% Compute rank
rank_block = rank(subs([M A_frictionless';A_frictionless zeros(c_frictionless,c_frictionless)], [m_l2, I_l2], [0,0]))